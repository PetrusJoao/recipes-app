...
<details>
  ...
  Como forma de organização das atividades e divizão das tarefas foi seguido o modelo de kanban, o grupo utilizou um [board pré-configurado disponibilizado pela trybe](https://trello.com/b/bhC6vLUH/group-30-trybe).
</details>
...
<details>
  ...
  Para o projeto ser validado, todos os testes de comportamento deveriam passar. Foi possível testar isso local rodando `npm run cy`. Esse comando roda a suite de testes do Cypress que valida se o fluxo geral e os requisitos funcionais estão funcionando como deveriam. Era possível também executar o comando `npm run cy:open` para ter um resultado visual dos testes executados.

  Esses testes não consideram o layout de maneira geral, mas sim os atributos e informações corretas. Os testes nos retornava uma mensagem de erro caso não estivessem passando (seja qual fosse o motivo).
  </details>

  <details><summary><strong>Executando testes de cobertura :open_umbrella:</strong></summary><br />
  Alguns requisitos pediam para desenvolver testes para sua aplicação. Esses testes foram avaliados através da cobertura de testes.

  Era possível verificar o percentual da cobertura de testes com o comando `npm run test-coverage`. 

  Era possível também executar `npm run test-coverage -- --collectCoverageFrom=caminho/da/Pagina` para verificar o percentual de cobertura de testes de cada 'Pagina'. Por exemplo, para verificar a cobertura de testes da página de `Login`, executou-se o comando `npm run test-coverage -- --collectCoverageFrom=src/pages/Login.js`.
  </details>
</details>
